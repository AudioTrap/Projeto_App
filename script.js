...
function abrirChatSonora() {
  document.getElementById("chat-sonora").style.display = "block";
}
...
function uploadVideo(event) {
  ...
  if (file && file.type.startsWith("video/")) {
    ...
    video.controls = true;
    addMessage(video, "user");
    respostaSonora("enviei um vídeo");
  }
}

function respostaSonora(pergunta) {
  let resposta = "Desculpe, ainda estou aprendendo a responder isso.";
  const texto = pergunta.toLowerCase();
  if (texto.includes("oi") || texto.includes("olá")) {
    resposta = "Olá! Eu sou a Sonora. Em que posso te ajudar?";
  } else if (texto.includes("detector")) {
    resposta = "O detector serve para identificar sons ao seu redor.";
  } else if (texto.includes("feedback")) {
    resposta = "Você pode enviar feedbacks sobre acessibilidade em locais pelo mapa.";
  }
